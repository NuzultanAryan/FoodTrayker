...
export default function LoginGuruScreen({ onLogin, onGoRegister, onBack }) {
  const [nip, setNip] = useState('');
  const [loading, setLoading] = useState(false);

  const handleLogin = async () => {
    if (!nip.trim()) { Alert.alert('Peringatan', 'NIP harus diisi!'); return; }
    setLoading(true);
    try {
      await signInWithEmailAndPassword(auth, `guru_${nip.trim()}@foodtrayker.com`, nip.trim());
      onLogin('guru');
    } catch (error) {
      Alert.alert('Gagal Login', 'NIP tidak ditemukan atau salah.');
