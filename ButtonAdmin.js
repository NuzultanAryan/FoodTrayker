import { BarChart2, Home, MessageSquare, Plus, User, Users, X } from 'lucide-react-native';
...
const CustomTabBarAdmin = ({ activePage, onTabPress }) => {
    ...
    <View style={styles.container}>
      {menuOpen && (
        <Animated.View style={[styles.subMenu, { opacity: fadeAnim, transform: [{ translateY }] }]}>
          <TouchableOpacity style={styles.subMenuItem} onPress={() => { toggleMenu(); onTabPress('daftar_siswa'); }}>
            <Users color="#FFFFFF" size={22} />
            <Text style={styles.subMenuLabel}>Daftar Siswa</Text>
          </TouchableOpacity>
        </Animated.View>
      )}
